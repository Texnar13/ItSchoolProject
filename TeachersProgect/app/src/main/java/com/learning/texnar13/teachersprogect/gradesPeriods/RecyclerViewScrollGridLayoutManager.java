...
public class RecyclerViewScrollGridLayoutManager extends RecyclerView.LayoutManager {

    private static final String TAG = RecyclerViewScrollGridLayoutManager.class.getSimpleName();
    ...
    /* количиство столбцов и колонок, находящихся в окне, а не за его пределами */
    ...
    /*
     * Вызывается из RecyclerView при запуске удаления view. Это вызывается
     * перед onLayoutChildren() в предварительной компоновке (pre-layout), если удаленные view не видны.
     * Мы используем его в этом случае для уведомления pre-layout о том, что произошло удаление.
     *
     * Этот метод по-прежнему вызывается, если удаленные view были видны, но это
     * произойдет ПОСЛЕ pre-layout.
     *
     *
     *
    ...
    /*
     * Этот метод является вашим первым вызовом из фреймворка. Он будет вызван, когда вам
     * нужно будет начать отрисовывать (laying out) первоначальный набор представлений. Этот метод не будет вызываться
     * повторно, поэтому не полагайтесь на него для постоянной обработки изменений во
     * время взаимодействия с пользователем.
     *
     * Этот метод будет вызываться при изменении набора данных в адаптере, поэтому его можно
     * использовать для обновления макета на основе нового количества элементов.
     *
     * Если включена прогнозирующая анимация(predictive animations), вы увидите, что это вызывается дважды. Во-первых, с
     * помощью функции state.isPreLayout(), возвращающей значение true, чтобы поместить дочерние эл-ты в их начальные условия.
     * Затем снова разложить дочерние эл-ты по их итоговым местам.
     *
     *
    ...
    /*
     * Вместо того, чтобы постоянно проверять, сколько views мы можем вместить
     * основываясь на смещениях scroll-а, мы упрощаем математику, вычисляя
     * видимую сетку как то, что изначально поместится на экране, плюс один.
     *
     *
     ...
     *
     * Этот метод описывает, как далеко, по мнению RecyclerView, содержимое должно прокручиваться горизонтально.
     * Вы несете ответственность за проверку границ границ и определение того, является ли этот свиток
     * событие каким-то образом требует добавления новых представлений или переработки старых представлений.
     */
    @Override
    ...
    /**
     * Шаблон для расширения LayoutParams для отслеживания строк/столбцов прикрепленных views.
     * Boilerplate to extend LayoutParams for tracking row/column of attached views.
     * <p>
     * Даже без extending LayoutParams мы должны переопределить этот метод
     * для предоставления параметров компоновки по умолчанию, которые каждый дочерний view
     * будет получать при добавлении.
     */
    @Override
